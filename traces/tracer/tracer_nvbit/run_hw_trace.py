...
for bench in benchmarks:
    ...
    for argpair in argslist:
        # Add -- Mahmood
        kernel_ids = []
        with open('kernel_ids.txt', 'r') as f:
            for line in f:
                key, val = line.strip().split(':')
                if exe == key:
                    kernel_ids = list(map(int, val.strip().split(' ')))
        # End -- Mahmood
        args = argpair["args"]
        run_name = os.path.join( exe, common.get_argfoldername( args ) )
        this_run_dir = os.path.abspath(os.path.expandvars(
            os.path.join(this_directory, "/disk1", "ncu2","traces","device-" + options.device_num, cuda_version, run_name)))
