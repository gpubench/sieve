...
void nvbit_at_init() {
  ...
  std::ifstream kid("ids.txt");
  if (kid) {
    std::string a;
    while (std::getline(kid, a)) {
      if(a.size() > 0)
        kernel_id.push_back(stoi(a)+1);
    }
  } else {
     printf("Unable to file find kernel_ids.txt\n");
     exit(1);
  }  
  // End -- Mahmood    
}
...
void instrument_function_if_needed(CUcontext ctx, CUfunction func) {
  ...
  for (auto f : related_functions) {
    ...
    for (auto instr : instrs) {
      ...
      /*if (!strcmp(instr->getOpcodeShort(), "LDGSTS")) {
          printf("Skipped LDGSTS,");
	      continue;
      }*/
      if (verbose) {
        instr->printDecoded();
  ...
  if (cbid == API_CUDA_cuMemcpyHtoD_v2) {
    ...
    if (!is_exit) {
      /*if (active_from_start && dynamic_kernel_limit_start && kernelid == dynamic_kernel_limit_start)
        active_region = true;*/
      // Add -- Mahmood
      printf("\nKernel %d -- %s\n", kernelid, nvbit_get_func_name(ctx, p->f, true));
      if (std::find(kernel_id.begin(), kernel_id.end(), kernelid) != kernel_id.end()) {
